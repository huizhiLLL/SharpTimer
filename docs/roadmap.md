...
SharpTimer 已具备可用的本地计时闭环，并已接入 MoYu32 系列智能魔方的 BLE 计时流程；GAN v2/v3/v4 与 QiYi 系列已加入第一版协议接入，等待实机验证和兼容性打磨。
...
- GAN v2/v3/v4 与 QiYi 系列的 Windows BLE 连接、加密通信、基础状态/电量/转动事件解析。

## 近期重点

- 实机验证 MoYu32、GAN v2/v3/v4 与 QiYi 的连接稳定性、断开重连、状态同步延迟和成绩保存体验。
...
- 在 MoYu32、GAN 与 QiYi 初步稳定后，继续扩展 Giiker、GoCube 等协议。
