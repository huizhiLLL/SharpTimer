...
- 主计时页蓝牙 Flyout：点击右上角圆形蓝牙按钮自动扫描，选择设备后进入智能魔方模式，并展示名称、电量和魔方状态预览。
- 智能魔方计时：MoYu32 首个有效转动会从空闲、观察或已停止状态进入复原计时；运行中继续接收转动并请求状态同步；当状态包回到复原态且本轮已经发生过转动时，自动停止计时、保存成绩并生成下一条打乱。连接初始状态即使是复原态也不会触发停止。

下一步应在实机上验证 MoYu32 最后一拧后的状态同步延迟，并根据体验决定是否在 App 侧维护本地魔方状态，减少每步后请求状态包的依赖。
