...
- `AppSettingsService`：WinUI 本地设置服务，保存是否启用观察、显示精度、主题偏好、背景材质和界面语言。该类设置不进入 SQLite schema。
- `LocalizedStrings`：App 层轻量中英文字符串表，用于导航、设置、成绩列表、计时状态和基础对话框文案。

默认本地偏好为英文界面、亮色主题、Mica 背景材质、百分秒显示和启用 15 秒观察。背景材质通过窗口级 `SystemBackdrop` 在 Mica、Mica Alt 和 Acrylic 之间切换。主界面页面切换使用轻量进入动画；如需严格使用 WinUI `DrillInNavigationTransitionInfo`，应先把各页面拆成独立 `Page` 或 `UserControl`，避免在运行时搬动已有 XAML 控件树。
...
- `WindowsBleSmartCubeConnector`：当前实现 MoYu32 连接、通知订阅、基础电量/状态/转动事件输出；MoYu32 对齐 cstimer 的 `moyu32cube.js`，优先使用广播厂商数据最后 6 字节倒序作为加密 MAC，取不到时使用 `WCU_MY32_5C3A` 这类设备名推导默认 MAC。连接时会逐个候选 MAC 请求强校验包，只有硬件、状态或转动包能确认 key，电量包不再作为 key 有效依据。
  ...
  断开连接时 App 会先停止扫描、摘除当前连接引用并忽略排队中的旧通知；底层连接会退订通知、关闭 CCCD，并释放 GATT service 与 BluetoothLEDevice，避免断开后仍继续解析旧包或后台请求状态。
- 主计时页蓝牙 Flyout：点击右上角圆形蓝牙按钮自动扫描，选择设备后进入智能魔方模式；连接后 Flyout 只展示已连接提示、名称、电量、重置本地魔方状态和断开连接操作。
- 主计时区智能魔方展示：连接智能魔方后，三维状态预览固定在主计时区右下角，不使用额外容器或说明文字。当前使用 WinUI 原生 `Canvas` 渲染，参考 DCTimer 的智能魔方状态图按 54 个贴纸做透视投影、深度排序、贴纸缩进、基础明暗和阴影；打乱文本只展示 move 序列，通过颜色区分下一步打乱和纠错步骤。
